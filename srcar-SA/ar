...
import { requestThreads } from '../../../redux/actions/threads';

function HomeContainer() {
  const { threadsData, loading } = useSelector(
    (state) => ({
  ...
  return (
    <div>
      ...
      {loading && <Loader width="91px" height="93px" />}
    </div>
  );
}
